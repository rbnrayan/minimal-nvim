...
local on_attach = function(client, bufnr)
    ...
    local vim_set_insertmode_keymap = function(keymap, action)
        vim.keymap.set('i', keymap, action, bufopts)
    end

    -- enable completion triggered by <c-x><c-o> | <S-Tab>
    vim.bo[bufnr].omnifunc = 'v:lua.vim.lsp.omnifunc'
    vim_set_insertmode_keymap('<S-Tab>', '<C-x><C-o>')
...
set_lsp('jdtls', {
    cmd = { vim.fn.expand('~/.nix-profile/bin/jdt-language-server'), '-configuration', vim.fn.expand('~/.cache/jdtls/config'), '-data', vim.fn.expand('~/.cache/jdtls/workspace') },
})
